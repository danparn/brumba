...
CodeMirror.defineMode("javascript", function (config, parserConfig) {
  ...
  function expect(wanted) {
    function exp(type) {
      ...
      if (wanted == ";" || type == "}" || type == ")" || type == "]") return pass();else
      return cont(exp);
    };
    return exp;
  }
  ...
  function statement(type, value) {
    ...
    if (type == "variable") {
      if (isTS && value == "declare") {
        ...
        cx.marked = "keyword";
        if (value == "enum") return cont(enumdef);else
        if (value == "type") return cont(typename, expect("operator"), typeexpr, expect(";"));else
  ...
  function expressionInner(type, value, noComma) {
    if (cx.state.fatArrowAt == cx.stream.start) {
      ...
      if (type == "(") return cont(pushcontext, pushlex(")"), commasep(funarg, ")"), poplex, expect("=>"), body, popcontext);else
  ...
  function maybeTarget(noComma) {
    return function (type) {
      ...
      if (type == "variable" && isTS) return cont(maybeTypeArgs, noComma ? maybeoperatorNoComma : maybeoperatorComma);else
  ...
  function mayberettype(type) {
    if (isTS && type == ":") {
      if (cx.stream.match(/^\s*\w+\s+is\b/, false)) return cont(expression, isKW, typeexpr);else
  ...
  return {
    ...
    indent: function indent(state, textAfter) {
      ...
      if (type == "vardef") return lexical.indented + (state.lastType == "operator" || state.lastType == "," ? lexical.info.length + 1 : 0);else
