...
import { strSplit, strCap, toJSON, objClone, decimals, objEmpty, timezone } from "./common.js";
import { $, e$, e$$, br, remote, createElement, modified, substArgs, translate } from "./util.js";
...
var eventMsg = 'Event blocked by handlers.';
...
export var formRetrieve = (form, id) => {
  remote({ db: br.db, coll: form.query.coll, where: { _id: id } }).then(res => {
    ...
    if (res[0]) {
      form.data = res[0];
      if (!$("form[name=".concat(form.name, "]")).dispatchEvent(new Event('retrieve', { cancelable: true }))) {
...
/**
    ...
    */
export var formInput = (form, fields, required) => {
  if (typeof form === 'string') form = findForm(form);
  if (!form) return null;

  var data = {};
  var flds = fields ? strSplit(fields, ',') : null;
  form.fields.forEach(f => {
    var inFields = flds && flds.includes(f.name);
...
export var formDelete = () => {
  ...
  if (form && form.data && form.data._id) {
    if (!$("form[name=".concat(form.name, "]")).dispatchEvent(new Event('delete', { cancelable: true }))) {
      return alert(eventMsg);
    }
    var par = { coll: form.query.coll, where: { _id: form.data._id } };
    if (form.query.db) {
      par.db = form.query.db;
    }
