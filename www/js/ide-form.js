...
function addFields() {
	...
	if ( form.hasClass('br-form') ) {
		...
		if ( form.hasClass('br-tabular') ) {
			var detail = form.find('.br-detail'),
				left = 0,
...
function addFlds( form, top, left, tabular ) {
	var header = form.find('.br-header')
		, detail = form.find('.br-detail')
...
function newField( id, top, left ) {
	var el = $('<label class="br-field" id="' + id + '" type="text" style="top:' + top + 'px;left:' + 
			left + 'px;">' + id + '</label>')
	setElement(el)
	return el
}
...
function setElement( el ) {
	...
	el.mousedown( function(ev) {
		if ( ev.which == 3 ) {
			ev.stopImmediatePropagation()
			setTimeout(function() {
				contextMenuIDE(el, ev.pageX, ev.pageY)
			}, 100)
		}
	}).click( function(ev) {
		ev.stopImmediatePropagation()
...
/* Context menu IDE
*/
function contextMenuIDE( elem, pageX, pageY ) {
	if ( elem.is('img') ) {
		var par = {
			items: [{id: 'file', text: 'From file'},
							{id: 'database', text: 'From database'}],
			pageX: pageX,
			pageY: pageY
		}
		contextMenu(par, function(ev, ui) {
			if ( ui.item[0].id == 'file' ) {
				fileUpload(br.app, function(res) {
					if ( res.dbret ) alert(translate('Cannot upload file'))
					else {
						elem.attr('data-id', res.newid)
						imgLoad(br.app, elem)
					}
				})
				
			} else if ( ui.item[0].id == 'database' ) {
				remote({cmd: 'GET', db: br.app, coll: 'fs.files', where: {contentType: {$regex:'^image/'}}}, function(res) {
					if ( res.dbret ) alert(res)
						...
						listBox('Files', dat, function(ui) {
							elem.attr('data-id', ui.item[0].id)
							imgLoad(br.app, elem)
						})
					}
				})
...
function newReport() {
	...
	foo.append(newField('DATE', 10, 50))
	foo.append(newLabel('Page:', 10, 0, '_page'))
	foo.append(newField('PAGE', 10, 0))
	pageNumPos()
}
